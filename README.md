...
The film and `hyprland-film-poster-20260908.jpg` now use actual September 8 recordings from the installed app. The 61.2-second edit shows the new Three.js opening, native tiling/resizing, keyboard trend-line placement and zoom, RSI, chart themes, and public Coinbase trade bubbles on the liquidity heatmap. Loading pauses are shortened; market pixels retain actual captured values. The logo titles and original instrumental music are added in the edit. Authoring sources and shot timings live in `work/marketing-video-v3/` in the local authoring workspace. The channel is https://www.youtube.com/@alpharch. The website has a VideoObject record matching the local MP4, date, duration and poster. Application changes are published through `48b387f`. The manual includes focused chart commands, keyboard drawing placement, native window resizing, saved chart themes and independent Omarchy window styling.
