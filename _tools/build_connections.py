...
STATUSES = {
    'account-test': ('Development · account test pending', 'The read-only adapter and chart integration have passed offline checks. Real gateway and account acceptance are still required; this is not a supported public broker release.'),
...
<aside><div class="aside-inner"><h2>Start with your feed.</h2><p>Your broker holds the account. Your feed supplies the prices. Sometimes they use different logins.</p><p>AMP, EdgeClear and other futures brokers can supply a route such as Rithmic or CQG. Read the broker page first, then follow the matching feed guide.</p><div class="aside-callout"><strong>IBKR runs on Linux.</strong><p>TWS and IB Gateway have official Linux downloads. The public preview includes an experimental read-only futures bridge. Real gateway and account testing are still required.</p><a href="/connections/interactive-brokers/">Read the IBKR guide →</a></div>
<h3>What works today?</h3><p>The existing tools read public Coinbase and Hyperliquid feeds and Deribit options. The 1.8.0-alpha.1 public preview includes verified Coinbase, Hyperliquid and Kraken desk feeds for BTC, ETH and SOL. The experimental IBKR bridge still needs account acceptance; broker execution is not available.</p><a href="#data-basics">Understand the data ↓</a></div></aside></div>
